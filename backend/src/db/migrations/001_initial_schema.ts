...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await db.schema
    ...
    .addColumn('document_id', 'uuid', (col) =>
      col.references('documents.id').onDelete('cascade').notNull().unique()
    )
  ...
  await db.schema
    .alterTable('document_tags')
    .addUniqueConstraint('document_tags_document_id_tag_unique', ['document_id', 'tag'])
    .execute()

  // Create indexes
  await db.schema
    .createIndex('idx_documents_hash')
    .on('documents')
    .column('file_hash')
    .execute()

  await db.schema
    .createIndex('idx_documents_category')
    .on('documents')
    .column('document_category')
    .execute()

  await db.schema
    .createIndex('idx_documents_extracted_date')
    .on('documents')
    .column('extracted_date')
    .execute()

  await db.schema
    .createIndex('idx_documents_folder')
    .on('documents')
    .column('folder_id')
    .execute()

  await db.schema
    .createIndex('idx_document_tags_tag')
    .on('document_tags')
    .column('tag')
    .execute()
  ...
  await db.schema
    .createIndex('idx_processing_jobs_target')
    .on('processing_jobs')
