...
export class JobService {
    ...
    async createJob(params: CreateJobParams): Promise<ProcessingJob> {
        ...
        return db.insertInto('processing_jobs').values(newJob).returningAll().executeTakeFirstOrThrow();
    }
     ...
     * Mark job as failed
     */
    async markJobFailed(jobId: string, errorMessage: string, attempts: number): Promise<ProcessingJob | undefined> {
        return this.updateJobStatus(jobId, {
            status: 'failed',
